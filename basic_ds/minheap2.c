...
int left_child(int);
int left_child(int i) { return (2 * i + 2); }

int right_child(int);
int right_child(int i) { return (2 * i + 2); }

int get_min(MinHeap *heap);
int get_min(MinHeap *heap) { return heap->arr[0]; }
...
MinHeap *insert_minheap(MinHeap *heap, int element) {
    if (heap->size == heap->capacity) {
        fprintf(stderr, "Cannot instert %d, Heap is already full!\n", element);
...
int main() {
    ...
    insert_minheap(heap, 15);
    insert_minheap(heap, 18);
    insert_minheap(heap, 33);
    insert_minheap(heap, 90);
